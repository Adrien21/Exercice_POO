<?php
    ...
    include_once("class_newsavis.php");
        ...
        $listeNews = $maCo->query('SELECT news.id, news.date, news.texte, news.titre, user.pseudo, jeuDlc.nom FROM news JOIN user ON idUser = user.id JOIN lien ON lien.id = news.idLien JOIN jeuDlc ON jeuDlc.id=lien.idJeuDlc WHERE jeuDlc.nom ="'.$jeuChoisi.'" ORDER BY news.date DESC');
